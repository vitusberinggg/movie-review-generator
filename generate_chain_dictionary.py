...
import pickle # Imports the pickle module to handle pickling and unpickling of Python objects

# --- Definitions ---

csv_path = "data/IMDB dataset.csv"
pickle_path = "data/chain-dictionary.pkl"
...
def build_chain_dictionary(reviews):

    """
    Builds a chain dictionary and a list of valid starting words.
    ...
    Returns:
        "chain_dictionary": {tuple(previous words): [possible next words]}
        "starters": list of starting words
    """

    chain_dictionary = {} # Creates an empty dictionary to hold the chain
    ...
    for review in reviews: # For each review in the list of reviews:
        ...
        for token_index in range(len(tokens) - order): # For each token index in the number of tokens - "order" (so we don't go out of bounds):
            ...
            if key not in chain_dictionary: # If the key is not already in the chain dictionary:
                chain_dictionary[key] = [] # Create a new entry in it for this key

            chain_dictionary[key].append(next_word) # Add the next word to the list of possible continuations for this key

    return chain_dictionary, starters, all_words

def load_reviews():

    """
    Loads the reviews from the CSV file.

    Arguments:
        None
    
    Returns:
        "reviews": A list of the reviews.
    """
...
def save_model(chain_dictionary, starters, all_words):

    """
    Saves the chain dictionary, the starters, and all words to a pickle file.
    ...
    with open(pickle_path, "wb") as file: # Opens the file in write-binary mode
        pickle.dump((chain_dictionary, starters, all_words), file) # Pickles and writes into file

reviews = load_reviews() # Loads the reviews from the CSV file by calling function "load_reviews"

chain_dictionary, starters, all_words = build_chain_dictionary(reviews) # Builds the chain dictionary, the list of valid starting words and the list of all words by calling function "build_chain_dictionary"

save_model(chain_dictionary, starters, all_words) # Saves the chain dictionary, the starters, and all words to a pickle file by calling function "save_model"